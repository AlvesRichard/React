...
const Promise = require("bluebird"); // se necesitaba para seedear
...
program
  ...
  .description(
    "Seeds the juke database with metadata from an XML iTunes library file. " +
...
function main() {
  ...
  program[TRACKS] = program.unlimited
    ? Infinity
    : program.limit || DEFAULT_TRACK_LIMIT;

  Promise.all([db.sync({ force: program.force }), checkItunes()])
    .spread((tablesAreReady, xmlPath) => {
      const imported = program.args
        .concat(
...
const Results = ["artists", "albums", "songs", "artistSong"].map((table) => {
  ...
  function findOrCreate(columnValues) {
    ...
    const pKeyExpr = (findOrCreate.primaryKey || ["id"])
      ...
      .join(",");
    var keys, values;
    let self = (findOrCreate[key] = Promise.props(columnValues)
      .then((cols) => {
